...
export class H264DataService {
  encoder: any;
  constructor() {
    this.encoder = new Mp3LameEncoder(8000, 128000);;
  }
  //設定PCM的頭
  ...
  getH264data(Bufferary, H264obj) {
    ...
    for (let num in m.gop) {
      ...
      if (H264obj.flag) {
        ...
        // feed audio data
        if (m.gop[num].a) {
          let pcm_data_tmp = this.decode(m.gop[num].a);
          let pcm_data = Array.from(pcm_data_tmp);
          var min = Math.min.apply(null, pcm_data);
          var input_range = Math.max.apply(null, pcm_data) - min;
          for (var i = 0; i < pcm_data.length; i++) {
            pcm_data[i] = (pcm_data[i] - min) * 2 / input_range + (-1);
          }
          H264obj.audio_buffer[H264obj.audio_buffer.length] = this.encoder.encode([pcm_data, pcm_data]);
        }
      }
    }//for (let num in m.gop)
    ...
    for (var i = 0; i < key_buffer_length; i++) {
      ...
      if (temp_audio_buffer[i]) {
        // console.log(temp_audio_buffer[i]);
        H264obj.current_audio = H264obj.current_audio.concat(Array.from(temp_audio_buffer[i]));
      }
