...
export default class JMuxmer extends Event {
    ...
    constructor(options) {
        ...
        this.remuxController.addTrack(this.options); // both, audio, video
    ...
    feed(data) {
        ...
        if (data.audio) {
            switch (this.options.audioType)
            {
                case 'aac':
                    audioFrames = AACParser.extractAAC(data.audio);
                    break;
                case 'mp3':
                    audioFrames = Mp3Parser.extractMp3(data.audio);
                    break;
            }
            if (audioFrames.length > 0) {
                chunks.audio = this.getAudioFrames(audioFrames, duration);
                // console.log(chunks.audio);
                remux = true;
            }
        }
        if (!remux) {
    ...
    getAudioFrames(audioFrames, duration) {
        let samples = [],
            units,
            sampleDuration,
            adjustDuration = 0;

        for (units of audioFrames) {
            samples.push({units});
        }
    ...
    createBuffer() {
        ...
        for (let type in this.remuxController.tracks) {
            ...
            if(type == 'audio')
            {
                if (!JMuxmer.isSupported('audio/mpeg')) {
                    debug.error('Browser does not support codec');
                    return false;
                }
                var sb = this.mediaSource.addSourceBuffer('audio/mpeg');
            }
            else if(type == 'video')
            {
