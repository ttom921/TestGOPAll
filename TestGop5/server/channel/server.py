...
class ChannelServer:
    ...
    def getDVRClient(self, chan, sckns):
        ...
        dvrsidlst = []
        if len(self.socketio.server.manager.rooms) == 0:
            return dvrsidlst
    ...
    def TestGenerateCarUserData(self):
        print("TestGenerateCarUserData-start")
        tcarnum = self._TestGenCarName()
        ...
        for x in tcarnum:
            lst = []
            for z in range(3):
                lst.append(tusername[z])
            self.dicCarUser[x] = lst
            index = index+1
        print(self.dicCarUser)
        print("TestGenerateCarUserData-end")
    # 產生測試的車子名字

    def _TestGenCarName(self):
        # print("_TestGenCarName-start")
        lst = []
        carnum = 2
    ...
    def GetAllChannelList(self):
        return self.channellst
    # 將DVR斷線   def disconnect(sid=None, namespace=None, silent=False):

    def DisconnectDVR(self, sid):
        ret = False
        myclient = self.clientmgr.FindDVRVlient(sid)
        if myclient != None:
            self.socketio.server.disconnect(myclient.sid, myclient.namespace)
            ret = True
        return ret
