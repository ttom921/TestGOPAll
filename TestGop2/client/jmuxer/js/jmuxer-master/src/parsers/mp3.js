...
export class Mp3Parser {
    ...
    static extractMp3(buffer) {
        ...
        // ------------------------------------------
        frameLength = Mp3Parser.getFrameLength(buffer);
        // ------------------------------------------


        if (!Mp3Parser.isMp3Pattern(buffer)) {
            debug.error('Invalid mp3 audio format');
            return result;
        }
        headerLength = Mp3Parser.getHeaderLength(buffer);
        if (!mp3Header) {
            mp3Header = buffer.subarray(0, headerLength);
        }        
        while (i < length) {
            frameLength = Mp3Parser.getFrameLength(buffer);
            // result.push(buffer.subarray(headerLength, frameLength));
            result.push(buffer.subarray(0, frameLength));
    ...
    setMp3Config() {
        let objectType,
            ...
            headerData = Mp3Parser.getMp3HeaderData;
        ...
        // this.track.codec = 'mp4a.40.2';
        this.track.codec = 'mp3';
